...
func TestCreatePost(t *testing.T) {
	...
	token, err := userInstance.Login(server.DB, user.Email, "password") //Note the password in the database is already hashed, we want unhashed
	if err != nil {
	...
	samples := []struct {
		...
		},
		{
			...
			statusCode:   500,
			tokenGiven:   tokenString,
			errorMessage: "title_already_taken",
		},
		{
...
func TestUpdatePost(t *testing.T) {
	...
	token, err := userInstance.Login(server.DB, PostUserEmail, PostUserPassword)
	if err != nil {
		log.Fatalf("cannot login: %v\n", err)
	}
	tokenString := fmt.Sprintf("Bearer %v", token)
